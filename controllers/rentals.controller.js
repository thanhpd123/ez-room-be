...
// const VALID_RENTAL_STATUSES = ['AVAILABLE', 'RENTED', 'HIDDEN', 'ARCHIVED'];
...
//         // TODO: Gửi notification cho chủ trọ nếu bị ẩn/khóa
//         if (newStatus === 'HIDDEN' || newStatus === 'ARCHIVED') {
//             // await notifyOwner(existing.users.id, `Bài đăng "${existing.title}" đã bị ${newStatus === 'HIDDEN' ? 'ẩn' : 'lưu trữ'}. Lý do: ${reason || 'Vi phạm quy định'}`);
//         }

//         return res.json({
//             success: true,
//             message: `Đã ${newStatus === 'HIDDEN' ? 'ẩn' : newStatus === 'AVAILABLE' ? 'hiển thị' : 'lưu trữ'} bài đăng "${rental.title}"`,
//             data: rental,
//         });
//     } catch (err) {
...
//             prisma.rental.count({ where: { status: 'ARCHIVED' } }),
//             prisma.rental.count({
//                 where: {
//                     createdAt: {
